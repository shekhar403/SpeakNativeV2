//
//  GoogleCloudTranslatorService.swift
//  SpeakNative
//
...
public class SwiftGoogleTranslate {
    ...
    private init() {
        self.apiKey = "AIzaSyA9TCll2Fdxv-mkQfR6ytUr21814qw-a-s"
        loadLanguages()
    }
    ...
    */
    private func loadLanguages() {
        if self.supportedLanguages.count == 0 {
            self.languages { (languages, error) in
                if let languages = languages {
                    self.supportedLanguages.append(contentsOf: languages)
                }
            }
        }
    }
    
    func loadLanguagesAsync() async throws -> [Language]? {
        guard var urlComponents = URLComponents(string: API.languages.url) else {
            fatalError("Error while fetching data")
        }
        
        var queryItems = [URLQueryItem]()
        queryItems.append(URLQueryItem(name: "key", value: "AIzaSyA9TCll2Fdxv-mkQfR6ytUr21814qw-a-s"))
        queryItems.append(URLQueryItem(name: "target", value: "en"))
    ...
    public func translateAsync(_ q: String, _ target: String, _ source: String, _ format: String = "text", _ model: String = "base") async throws -> String {
    ...
    */
    ...
    // Used for testcase
    public func detectAsync(_ q: String) async throws -> String {
        guard var urlComponents = URLComponents(string: API.detect.url) else {
            return ""
        }
